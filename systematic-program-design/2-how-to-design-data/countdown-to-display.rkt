#lang racket

(require test-engine/racket-tests)
(require 2htdp/image)

; PROBLEM: You are asked to contribute to the design for a very simple New
; Year's Eve countdown display. You already have the countdown definition given
; below. You need to design a function that consumes CountDown and produce an
; image showing the current status of the countdown.


;; Data Definitions:

;; CountDown is one of:
;; - false
;; - Natural[1, 10]
;; - "complete"
;; interp.
;;  false means countdown has not yet started.
;;  Natural[1, 10] means countdown is running and how many seconds left.
;;  "complete" means countdown is over.
;; Examples:
(define CD1 false)
(define CD2 7)
(define CD3 "complete")

;(define (fn-for-count-donw cd) 
  ;(cond [(false? cd) (...)] 
        ;[(string=? cd "complete") (...)]
        ;[(and (number? cd) (>= cd 1) (<= x 10)) (... cd)]))

;(define (fn-for-count-donw cd) 
  ;(cond [(false? cd) (...)] 
        ;[(number? cd) (... cd)]
        ;[else (...)]))

;; Template rules used:
;; - one of: 3 cases
;; - atomic distinct: false
;; - atomic distinct: "complete"
;; - atomic non-distinct: Natural[1, 10]


;; Functions:
;; CountDown -> Image
;; Produce nice image of current state of countdown.
;(define (countdown-to-image c) (square 0 "solid" "white")) ; stub

(check-expect (countdown-to-image false) (square 0 "solid" "white"))
(check-expect (countdown-to-image 5) (text (number->string 5) 24 "black"))
;(check-expect (countdown-to-image 1) (text (number->string 1) 24 "black"))
;(check-expect (countdown-to-image 10) (text (number->string 10) 24 "black"))
(check-expect (countdown-to-image "complete") (text "Happy New Year!" 24 "red"))

; template get from the CountDown data definition

(define (countdown-to-image c) 
  (cond [(false? c) (square 0 "solid" "white")] 
        [(number? c) (text (number->string c) 24 "black")]
        [else (text "Happy New Year!" 24 "red")]))

(test)







